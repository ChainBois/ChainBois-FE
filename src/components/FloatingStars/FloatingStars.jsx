...
const FloatingStars = ({
	...
	const getSizeRange = () => {
		switch (size) {
			case 'small':
				return { min: 1.2, max: 2.0 } // vw units: 2.0vw to 2.0vw
			case 'medium':
				return { min: 1.5, max: 2.5 } // vw units: 1.5vw to 2.5vw
			case 'large':
				return { min: 2.0, max: 3.5 } // vw units: 2.0vw to 3.5vw
			case 'auto':
			default:
		...
		>
			...
			<div
				style={{
					...
					height: '280px',
					background:
						'linear-gradient(135deg, rgba(40, 10, 10, 0.9), rgba(20, 5, 5, 0.9))',
					borderRadius: '20px',
			...
			>
				...
				>
					...
					>
						A rare drop from the cosmic vault. The stars blink around this
						prized possession, signaling its extraordinary power and
						magnificence.
					</p>
